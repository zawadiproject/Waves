...
class TradeBalanceAndRoundingTestSuite
  ...
  "cannon place order with more then 8 non zero digits in price" in {
    val bobOrder1 = matcherNode.prepareOrder(bobNode, wctWavesPair, OrderType.SELL, 0, 0)
    assertBadRequest(matcherNode.placeOrder(aliceNode, wctWavesPair, BUY, 1234567891, 1000000000));
  }

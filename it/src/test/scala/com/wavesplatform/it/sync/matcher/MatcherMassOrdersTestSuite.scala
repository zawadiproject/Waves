...
class MatcherMassOrdersTestSuite extends MatcherSuiteBase {
  ...
  "Create orders with statuses FILL, PARTIAL, CANCELLED, ACTIVE" - {
    ...
    val aliceOrderIdFill = matcherNode
      .placeOrder(aliceAcc, aliceSecondWavesPair, OrderType.SELL, 3, Order.PriceConstant, minMatcherFee, orderVersion, 10.minutes)
      .message
      .id
    ...
    val aliceOrderToCancelId =
      matcherNode
        .placeOrder(aliceAcc, aliceSecondWavesPair, OrderType.SELL, 3, Order.PriceConstant, minMatcherFee, orderVersion, 70.seconds)
        .message
        .id

    val aliceActiveOrderId = matcherNode
      .placeOrder(aliceAcc, aliceSecondWavesPair, OrderType.SELL, 3, Order.PriceConstant + 100000000, minMatcherFee, orderVersion, 10.minutes)
      .message
      .id
  ...
  private def ordersRequestsGen(n: Int, sender: PrivateKeyAccount, assetPair: AssetPair, orderType: OrderType, amount: Long): Seq[String] = {
    val orderIds = 1 to n map (_ => {
      matcherNode
        .placeOrder(sender, assetPair, orderType, amount, Order.PriceConstant, minMatcherFee, orderVersion, (120 + Random.nextInt(70)).seconds)
        .message
        .id
    })
    orderIds

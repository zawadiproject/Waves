...
package object smartcontract {
  ...
  def exchangeTx(buyer: PrivateKeyAccount,
                 seller: PrivateKeyAccount,
                 matcher: PrivateKeyAccount,
                 assetPair: AssetPair,
                 buyFee: Long = 300000L,
                 sellFee: Long = 300000L,
                 mf: Long = 300000L): JsObject = {
    val sellPrice   = (0.50 * Order.PriceConstant).toLong
    val (buy, sell) = orders(2, buyer, seller, matcher, assetPair, buyFee, sellFee)
    ...
    val tx = ExchangeTransactionV2
      .create(
        ...
        amount = amount,
        price = sellPrice,
        buyMatcherFee = (BigInt(buyFee) * amount / buy.amount).toLong,
        sellMatcherFee = (BigInt(sellFee) * amount / sell.amount).toLong,
        fee = mf,
  ...
  def orders(version: Byte = 2,
             ...
             assetPair: AssetPair,
             buyFee: Long = 300000L,
             sellFee: Long = 300000L): (Order, Order) = {
    ...
    val buyAmount           = 2
    val sellAmount          = 3
    val buy                 = Order.buy(buyer, matcher, assetPair, buyAmount, buyPrice, time, expirationTimestamp, buyFee, version)
    val sell                = Order.sell(seller, matcher, assetPair, sellAmount, sellPrice, time, expirationTimestamp, sellFee, version)

...
trait Signed extends Authorized {
  final protected val signatureValid: Coeval[Boolean] = Coeval.now(true)
  @ApiModelProperty(hidden = true)
  final protected val signedDescendants: Coeval[Seq[Signed]] = Coeval.evalOnce(Seq.empty)
  @ApiModelProperty(hidden = true)
  final protected val signaturesValidMemoized: Task[Either[InvalidSignature, this.type]] = Signed.validateTask[this.type](this).memoize
  @ApiModelProperty(hidden = true)
  final val signaturesValid: Coeval[Either[InvalidSignature, this.type]] =
    Coeval.now(Right(this)) // .evalOnce(Await.result(signaturesValidMemoized.runAsync(), Duration.Inf))
}
...
//  }
  ...
  private def validateTask[S <: Signed](s: S): Task[E[S]] = Task.now(Right(s))
//    Task {
...
//    }.flatten
  ...
  def validateOrdered[S <: Signed](ss: Seq[S]): E[Seq[S]] = Right(ss)
//    Await.result(Task
